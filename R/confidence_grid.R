...
confidence_grid <- function(alpha, lower_bds, upper_bds, seeds, G, s_obs, tol, res, t_init = NULL, T_stat = ma_depth) {
  ...
  for (j in 1:p) {
    ...
    confidence_interval <- get_CI(alpha, lower_bds, upper_bds, j, seeds, G, s_obs, tol, t_init, T_stat, print_info = FALSE, check_input)
  ...
  update_array <- function(indices, indicator_array) {
    ...
    if (p_value(cube_lower_bds, cube_upper_bds, seeds, G, s_obs, mid_point, T_stat, print_info = FALSE, check_input = FALSE)$p_val > alpha) {
...
#' @return A one-dimensional indicator vector or a two-dimensional indicator array ready for visualization.
#' @examples
#' ### simple projection
